...
local t = Def.ActorFrame{
	LoadFont(ThemePrefs.Get("ThemeFont") == "Common" and "Wendy/_wendy white"
			...
			or ThemePrefs.Get("ThemeFont") .. " Bold")..{
		Text="GAME",
		InitCommand=function(self) self:xy(_screen.cx,_screen.cy-40):croptop(1):fadetop(1):zoom((ThemePrefs.Get("ThemeFont") == "Common" or ThemePrefs.Get("ThemeFont") == "Unprofessional") and 1.2 or 1.95):shadowlength(1) end,
...
for player in ivalues(Players) do
	...
	if PROFILEMAN:IsPersistentProfile(player) then
		...
		for i,stat in ipairs(profile_stats) do
			PlayerStatsAF[#PlayerStatsAF+1] = LoadFont(ThemePrefs.Get("ThemeFont") .. " Normal")..{
				Text=stat,
				InitCommand=function(self)
